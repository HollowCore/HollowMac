...
CTEST(HCView, Geometry) {
    ...
    HCPoint offsetOrigin = HCPointTranslate(HCViewOrigin(view), -5.0, 10.0);
    HCViewSetOrigin(view, offsetOrigin);
    ASSERT_TRUE(HCPointIsEqual(HCViewCenter(view), HCPointTranslate(offsetOrigin, HCViewSize(view).width * 0.5, HCViewSize(view).height * 0.5)));
    ...
    HCPoint offsetCenter = HCPointTranslate(HCViewCenter(view), 20.0, -7.0);
    HCViewSetCenter(view, offsetCenter);
    ASSERT_TRUE(HCPointIsEqual(HCViewCenter(view), offsetCenter));
    ASSERT_TRUE(HCPointIsEqual(HCViewOrigin(view), HCPointTranslate(offsetCenter, HCViewSize(view).width * -0.5, HCViewSize(view).height * -0.5)));
    ASSERT_TRUE(HCSizeIsEqual(HCViewSize(view), originalFrame.size));
